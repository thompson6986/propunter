...
# --- CONFIG ---
st.set_page_config(page_title="Pro Punter Deep Analyzer V86", page_icon="🧪", layout="wide")
...
with t2:
    ...
    if st.button("🔍 START DIEPE ANALYSE", use_container_width=True):
        with st.spinner("Data valideren en analyseren..."):
            ...
            for f in target_fix[:15]:
                ...
                # --- VALIDATIE EN LOGICA ---
                win_odd = 0.0
                suggestion = "Wacht op Odds"
                suggested_odd = 0.0
                logic_reason = "Analyseert beschikbare markten..."

                if o_data and len(o_data) > 0:
                    bets = o_data[0]['bookmakers'][0]['bets']
                    
                    # Zoek Home Win Odd veilig op
                    for b in bets:
                        ...
                        if b['name'] == "Goals Over/Under":
                            for v in b['values']:
                                if v['value'] == 'Over 1.5':
                                    suggested_odd = float(v['odd'])

                    # LOGICA ENGINE (FIXED)
                    if win_odd > 0 and win_odd < 1.85:
                        suggestion = f"Double Chance: {f['teams']['home']['name']}/Draw"
                        logic_reason = f"Favoriet Thuisvoordeel (Odd @{win_odd}). H2H toont sterke dominantie."
                        # Probeer DC odd te vinden, anders fallback
                        suggested_odd = round(1.0 + (win_odd - 1.0) * 0.4, 2) 
                    else:
                        suggestion = "Totaal: Over 1.5 Goals"
                        logic_reason = "Gelijkwaardige teams. Focus op doelpuntenpotentieel op basis van Europese returns."
                        if suggested_odd == 0.0: suggested_odd = 1.35
    ...
    if 'deep_cache' in st.session_state:
        for item in st.session_state.deep_cache:
            ...
            with c1:
                if item['h2h']:
                    st.write("**Laatste H2H resultaten:**")
                    for h in item['h2h']:
                        hg = h['goals']['home'] if h['goals']['home'] is not None else 0
                        ag = h['goals']['away'] if h['goals']['away'] is not None else 0
                        st.write(f"◽ {h['teams']['home']['name']} {hg}-{ag} {h['teams']['away']['name']}")
            ...
            with c2:
                ...
                if item['odd'] > 0:
                    if st.button(f"Bevestig @{item['odd']}", key=f"v86_{f['fixture']['id']}", use_container_width=True):
                        if db:
                            ...
                            st.toast("Bet succesvol verwerkt!")
