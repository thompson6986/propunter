...
with t1:
    ...
    if st.button("🚀 ZOEK PROFESSIONELE WEDDENSCHAPPEN", use_container_width=True):
        try:
    ...
    for i, slip in enumerate(st.session_state.gen_slips[:4]):
        if len(slip) == m_count:
            ...
            stake = col_a.number_input(f"Inzet (€)", min_value=1.0, max_value=float(st.session_state.balance), value=10.0, key=f"stk_{i}")
            
            if col_b.button(f"💾 PLAATS @{t_odd}", key=f"pl_{i}", use_container_width=True):
                if st.session_state.balance >= stake:
                    st.session_state.balance -= stake
                    if db:
                        db.collection("saved_slips").add({
                            "user_id": u_id, "timestamp": datetime.now(TIMEZONE), "total_odd": t_odd, "matches": slip, "stake": stake
                        })
                        st.success("Bet geplaatst!")
                        time.sleep(1)
                        st.rerun()
            st.markdown('</div>', unsafe_allow_html=True)
...
with t2:
    ...
    if db:
        try:
            ...
            if docs:
                f_ids = [m['fixture_id'] for d in docs for m in d.to_dict()['matches']]
                live_updates = {}
                if f_ids:
        ...
        except Exception as e: st.warning("Database synchroniseert...")
