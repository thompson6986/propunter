...
st.markdown("""
    ...
    .start-time { color: #8b949e; font-size: 0.85rem; font-family: monospace; }
...
# --- TAB 1: GENERATOR (Met behoud van jouw instellingen) ---
with t1:
    ...
    if st.button("🚀 GENEREER PROFESSIONELE SLIPS", use_container_width=True):
        ...
        except: st.error("Fout bij laden data.")
    ...
    for i, slip in enumerate(st.session_state.gen_slips[:4]):
        ...
        for m in slip:
            t_odd *= m['odd']
            st.markdown(f'<div class="match-row"><div><span class="prob-badge" style="color:#3fb950;">{m["prob"]}%</span><div style="color:#c9d1d9; font-weight:bold;">{m["match"]}</div><div class="start-time">🕒 {m["time"]} | {m["market"]}</div></div><div class="score-badge">@{m["odd"]}</div></div>', unsafe_allow_html=True)
...
# --- TAB 2: TRACKER (Hersteld met Tijd & Klok) ---
with t2:
    st.markdown(f"### 📡 Portfolio Tracker | Saldo: €{st.session_state.balance:.2f}")
    if db:
        try:
            # Query exact volgens je Descending Index
            docs = db.collection("saved_slips").where("user_id", "==", u_id).order_by("timestamp", direction=firestore.Query.DESCENDING).limit(15).get()
            
            if docs:
                all_f_ids = list(set([m['fixture_id'] for d in docs for m in d.to_dict()['matches']]))
                live_map = {}
                if all_f_ids:
                    res = requests.get(f"{BASE_URL}/fixtures", headers=headers, params={'ids': "-".join(map(str, all_f_ids))})
                    if res.status_code == 200:
                        for f in res.json().get('response', []):
                            live_map[f['fixture']['id']] = f
                ...
                for doc in docs:
                    ...
                    for m in s['matches']:
                        ...
                        # Linkerkant: Match info & Starttijd
                        st.markdown(f'<div><div style="color:#c9d1d9; font-weight:bold;">{m["match"]}</div><div class="start-time">🕒 {m.get("time", "Live")} | {m["market"]} (@{m["odd"]})</div></div>', unsafe_allow_html=True)
                        
                        # Midden: Live Klok / Status
                        if f_upd:
                            status = f_upd['fixture']['status']['short']
                            elapsed = f_upd['fixture']['status']['elapsed']
                            if status in ['1H', '2H', 'HT']:
                                st.markdown(f'<div><div class="live-indicator">🔴 {elapsed}\'</div></div>', unsafe_allow_html=True)
                            elif status == 'FT':
                                won_matches += 1
                                st.markdown('<div><div style="color:#3fb950; font-weight:bold;">🏁 FT</div></div>', unsafe_allow_html=True)
                            else:
                                st.markdown(f'<div><div style="color:#8b949e;">NS</div></div>', unsafe_allow_html=True)
                        
                        # Rechterkant: Score
                        st.markdown(f'<div class="score-badge">{h_g} - {a_g}</div></div>', unsafe_allow_html=True)
                    
                    # Cash-out berekening & knoppen
                    co_val = round((s['stake'] * s['total_odd']) * (won_matches / len(s['matches'])) * 0.88, 2)
                ...
                st.info("Geen actieve slips in portfolio.")
        except Exception as e:
            st.error(f"Synchronisatie fout: {e}")
